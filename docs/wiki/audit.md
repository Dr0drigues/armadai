...
- **The prompt measured is the whole body**, in the order a linked config lays it out: `## System Prompt`, then `## Instructions`, `## Output Format`, `## Context`. Measured: 60 of the 77 agents carry `## Instructions` and 44 carry `## Output Format`, so counting only the first section understates most of the library. It is still bounded by what the shared parser exposes, which is currently less than the files hold — see [#392](https://github.com/Dr0drigues/armadai/issues/392), where a `###` sub-heading silently truncates a section for `link` and `run` as well as for the audit.
...
Measured end to end on the 77-agent library that motivated this: **1 `A01`** (a real `armadai new` stub that was never filled in and that `armadai run` cannot load) and **3 `A07`** Info findings on Java/Node twin agents. Nothing else — which is the point: a rule that fires 77 times on a healthy library is not a finding, it is a reader bug.
