...
pub(crate) fn render_agent(agent: &ImportedAgent) -> String {
    ...
    let description = agent
        ...
        .description
        .as_deref()
        .unwrap_or("Imported by `armadai audit --propose`.");
...
pub fn generate_proposal(root: &Path, config: &ImportedConfig) -> anyhow::Result<ProposalSummary> {
    ...
    let result: anyhow::Result<ProposalSummary> = (|| {
        ...
        let _ = writeln!(
            pack,
            "description: \"Generated by `armadai audit --propose` from the audited configuration\""
        );
...
";
    ...
    fn generate_proposal_produces_valid_installable_pack() {
        ...
        // The pack must not claim a source it does not have: since #391 the
        // audited configuration can be ArmadAI-format agents from
        // `~/.config/armadai/agents` as well as native Claude Code files, and
        // a pack.yaml saying "from the native Claude Code configuration" was
        // false for 76 of the 76 agents of one real `--global --propose` run.
        let pack = std::fs::read_to_string(out.join("pack.yaml")).unwrap();
        assert!(
            pack.contains("from the audited configuration"),
            "the pack must name its source truthfully:\n{pack}"
        );
        assert!(
            !pack.contains("native Claude Code"),
            "and must not claim a format it may not have read:\n{pack}"
        );
