...
/// Generate `AGENTS.md` with only the team table (no coordinator system prompt).
///
/// Used when a root `AGENTS.md` already exists in the project, to avoid duplicating
/// or conflicting with the user-maintained instructions. Only the `## Team` section
/// is emitted, prefixed with an auto-generated comment.
pub fn generate_coordination_only(_coordinator: &LinkAgent, agents: &[LinkAgent]) -> OutputFile {
    let mut content = String::new();
...
mod tests {
    ...
    fn test_generate_coordination_only_structure() {
        ...
        assert_eq!(file.path, PathBuf::from(".codex/AGENTS.md"));
        ...
        // Must contain Team section
        assert!(file.content.contains("## Team"));
        // Must list agents with TOML config links
        assert!(file.content.contains("| Vigie |"));
        assert!(file.content.contains("agents/vigie.toml"));
        assert!(file.content.contains("| Charpentier |"));
        assert!(file.content.contains("agents/charpentier.toml"));
    }
