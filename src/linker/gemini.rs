...
/// Generate `GEMINI.md` with only the coordination/team sections (no system prompt).
///
/// Used when a root `GEMINI.md` already exists in the project, to avoid duplicating
...
mod tests {
    ...
    fn test_generate_coordination_only_structure() {
        ...
        assert_eq!(file.path, PathBuf::from(".gemini/GEMINI.md"));
        ...
        // Must list agents with file links
        assert!(file.content.contains("| Vigie |"));
        assert!(file.content.contains("agents/vigie.md"));
        assert!(file.content.contains("| Charpentier |"));
        assert!(file.content.contains("agents/charpentier.md"));
        // Must mention complex tasks
        assert!(file.content.contains("complex tasks"));
    }
