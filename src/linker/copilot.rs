...
/// Generate `.github/copilot-instructions.md` with only the team table (no system prompt).
///
/// Used when a root `.github/copilot-instructions.md` already exists in the project,
/// to avoid duplicating or conflicting with the user-maintained instructions. Only the
/// `## Team` section is emitted, prefixed with an auto-generated comment.
...
mod tests {
    ...
    fn test_generate_coordination_only_structure() {
        ...
        assert_eq!(file.path, PathBuf::from(".github/copilot-instructions.md"));
        ...
        assert!(file.content.contains("## Team"));
        // Agents are listed by slug in copilot format
        assert!(file.content.contains("| vigie |"));
        assert!(file.content.contains("| charpentier |"));
        // Must include the @<agent-name> delegation instruction
        assert!(file.content.contains("@<agent-name>"));
    }
