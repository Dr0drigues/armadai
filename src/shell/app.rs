...
fn accumulate_pipeline_metrics(
    ...
    resp: &armadai_providers::json_runner::CliResponse,
) {
...
async fn event_loop(
    ...
    loop {
        ...
        let is_json_mode = armadai_providers::json_runner::supports_json(&cmd);
        ...
        loop {
            ...
            let mut result_event: Option<armadai_providers::json_runner::CliResponse> = None;
            ...
            while let Ok(line) = stream_rx.try_recv() {
                ...
                if is_json_mode {
                    use armadai_providers::json_runner::{StreamEvent, parse_stream_event};
...
async fn execute_tandem(
    ...
    struct ProviderStream {
        ...
        result_event: Option<armadai_providers::json_runner::CliResponse>,
    ...
    while active_streams > 0 {
        ...
        for stream in &mut streams {
            while let Ok(line) = stream.stream_rx.try_recv() {
                ...
                let is_json_mode = armadai_providers::json_runner::supports_json(&stream.cmd);
                if is_json_mode {
                    use armadai_providers::json_runner::{StreamEvent, parse_stream_event};
...
async fn execute_pipeline_steps(
    ...
    for (i, step) in steps.iter().enumerate() {
        ...
        let is_json_mode = armadai_providers::json_runner::supports_json(&resolved.cmd);
        let mut step_result_event: Option<armadai_providers::json_runner::CliResponse> = None;
...
mod tests {
    use super::*;
    use armadai_providers::json_runner::CliResponse;
